...
class Data():
    ...
    def build_orginal_trace_list(self):
        ...
        for trace in self.orginal_list:
            # print(trace)
            src_temp = [self.event2id[record[0]] for record in trace]
            tgt_temp = [float((datetime.datetime.strptime(trace[-1][1], '%Y-%m-%d %H:%M:%S') -
                               datetime.datetime.strptime(record[1],'%Y-%m-%d %H:%M:%S')).seconds / 60 / 60)
                        for record in trace]
